...
from kosmorrolib.data import ASTERS, AsterEphemerides, MoonPhase, Event
...
class TextDumper(Dumper):
    ...
    def get_events(self, events: [Event]) -> str:
        def get_event_description(ev: Event):
            obj_names = tuple(obj.name for obj in ev.objects)
            print(ev.event_type, obj_names)
            description = strings.from_event(ev) % obj_names
...
class _LatexDumper(Dumper):
    ...
    def add_strings(
        ...
        for aster in ASTERS:
            document = document.replace(
                "+++ASTER_%s+++" % aster.skyfield_name.upper().split(" ")[0], aster.name
            )
